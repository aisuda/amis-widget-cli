...
export default defineConfig(({ command, mode }) => {
  ...
  if (command === 'serve') {
    ...
    const curDir = process.env.UNI_BUILD_DIR
      ? '/' + process.env.UNI_BUILD_DIR
      : '';
    const curEntry = path.resolve(
      __dirname,
      `./build${curDir}/${process.env.UNI_BUILD_LIB || 'renderer'}.ts`,
    ); // 构建自定组件入口文件
      ...
      return {
        ...
        build: {
          ...
          lib: {
            ...
            fileName: (format) =>
              `${process.env.UNI_BUILD_LIB || 'renderer'}.${format}.js`,
          },
