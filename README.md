...
4. **关于amis-widget-cli的配置文件**
  1. 提供全量的默认配置，实现零配置、开箱即用的能力
  2. 自定义构建配置，请在当前项目根目录创建amis-widget-cli配置文件（amis.config.js），amis-widget-cli提供初始化配置文件的方法：
    ```bash
    $ amis config init
    ```
  3. amis.config.js为当前项目的配置文件，优先级最高（可覆盖amis-widget-cli提供的默认配置）

5. **配置构建入口文件（webpack.entry）**
  1. 默认的构建入口文件: ./src/index.js
  2. 自定义构建入口(amis.config.js中提供对应的配置入口)
...
6. **关于多页面**
  1. 当amis.config.js的entry只有一个入口配置，且对应的构建入口文件不存在时，会自动从src/pages中获取构建入口（支持多页面多模板）
...
7. **关于多页面多模板**
  ...
  3. 当项目中./src/index.html不存在时，会使用默认页面模板
